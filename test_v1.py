...
# Store layers weight & bia

weights = {
    # 5x5 conv, 1 input, 16 outputs
    'wc1': weight_variable([5, 5, 1, 16]),
    # 3x3 conv, 16 inputs, 48 outputs
    'wc2': weight_variable([5, 5, 16, 16]),
    'wc3': weight_variable([3, 3, 16, 32]),
    'wc4': weight_variable([3, 3, 32, 32]),
    'wc5': weight_variable([2, 2, 32, 64]),
    'wc6': weight_variable([3, 3, 64, 64]),

    # fully connected, 2*2*64 inputs, 100 outputs
    'wd1': weight_variable([2 * 2 * 64, 128]),

    'wd2': weight_variable([128, 128]),
    # 100 inputs, 68*2 outputs (class prediction)
    'out': weight_variable([128, 6])
}
...
biases = {
    ...
    'bc4': bias_variable([32]),
    'bc5': bias_variable([64]),
    'bc6': bias_variable([64]),

    'bd1': bias_variable([128]),
    'bd2': bias_variable([128]),
    'out': bias_variable([6])
}
...
image = tf.placeholder(tf.float32, shape=[None, 244, 244])
...
# Reshape input picture
x_image = tf.reshape(image, [-1, 244, 244, 1])
# Layer1
h_conv1 = tf.abs(tf.nn.relu(conv2d(x_image, weights['wc1']) + biases['bc1']))
h_pool1 = max_pool_2x2(h_conv1)

# Layer2
h_conv2 = tf.abs(tf.nn.relu(conv2d(h_pool1, weights['wc2']) + biases['bc2']))
h_pool2 = max_pool_2x2(h_conv2)

# Layer3
h_conv3 = tf.abs(tf.nn.relu(conv2d(h_pool2, weights['wc3']) + biases['bc3']))
h_pool3 = max_pool_2x2(h_conv3)
h_conv4 = tf.abs(tf.nn.relu(conv2d(h_pool3, weights['wc4']) + biases['bc4']))
h_pool4 = max_pool_2x2(h_conv4)
h_conv5 = tf.abs(tf.nn.relu(conv2d(h_pool4, weights['wc5']) + biases['bc5']))
h_pool5 = max_pool_2x2(h_conv5)
h_conv6 = tf.abs(tf.nn.relu(conv2d(h_pool5, weights['wc6']) + biases['bc6']))
h_pool6 = max_pool_2x2(h_conv6)
# Fully connected layer
# Reshape conv4 output to fit fully connected layer input
h_pool5_flat = tf.layers.flatten(h_pool6)
h_fc1 = tf.abs(tf.nn.tanh(tf.matmul(h_pool5_flat, weights['wd1']) + biases['bd1']))
# h_fc2 = tf.abs(tf.nn.tanh(tf.matmul(h_fc1, weights['wd2']) + biases['bd2']))

# h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

# Output, landmark regression
y_landmark = tf.matmul(h_fc1, weights['out']) + biases['out']
W_fc_landmark = weights['out']

loss = tf.sqrt(tf.reduce_mean(tf.reduce_sum(tf.square(landmark - y_landmark))))

# L2 regularizers for fully connected layers (shared parameter regularization)
regularizers = 1 * (tf.nn.l2_loss(weights['wd1']) + tf.nn.l2_loss(biases['bd1']) +\
...
#  \
cost = loss + regularizers

global_step = tf.Variable(0, trainable=False)
learning_rate = tf.train.exponential_decay(0.01,  global_step  , 25, 0.9, staircase=True)
...
with tf.Session() as sess:
    ...
    if train_Flag:
        ...
        for x in range(1000000):
                ...
                sess.run([optimizer] ,feed_dict={image: training_imgs, landmark: training_landmarks, keep_prob: 1})
                ...
                if x % 5 == 0:
                    loss_func, reg, lr = sess.run([cost, regularizers, learning_rate],\
                                             feed_dict={image: training_imgs, landmark: training_landmarks, keep_prob: 1})

                    print('Num of step : ' + str(x) + '/1000000')
                    print('train_cost_func : ' + str(loss_func) + ', reg_er: ' + str(reg) + ', lr : ' + str(lr))
                    print ('loss er: : ' + str(loss_func - reg))
                ...
                if x % 500 == 0 and x is not 0:
                    my_totoal_loss.append(loss_func)
